...
class Contract:
    ...
    def royalties(self, new_royalties):
        if isinstance(new_royalties, int):
            ...
            raise Exception("No royalties")

    @classmethod
    def contracts_by_date(cls, new_date):
        dates = [ contract for contract in Contract.all if contract.date == new_date ]
        return sorted(dates, key= lambda contract: contract.date)
